...
import { useUpdateProductMutation } from "@/redux/features/project/project-api"

const EditProjectForm = ({ initialData }: { initialData: IProject }) => {
  ...
  const onSubmit = async (values: z.infer<typeof createProjectFormSchema>) => {
    try {
      const res = await updateProduct({ id: initialData.id, ...values })
      if (res.data?.success) {
          ...
          <div className="px-10">
            ...
            <div className="flex gap-3 flex-wrap">
              {form.watch("photos").map((photo: string) => (
                <div className="relative group border border-border rounded-md overflow-hidden">
                  <Button
                    variant="destructive"
                    size="icon"
                    type="button"
                    ...
                    onClick={e => {
                      ...
                      form.setValue(
                        "photos",
                        form.watch("photos").filter(img => img != photo)
                      )
                    }}
                  >
